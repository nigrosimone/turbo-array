...
const lFilterMap = turbo<number>().filter(f).map(m).build();

const lFilterReduce = turbo<number>().filter(f).reduce(r, 0).build();

const lFilterJoin = turbo<number>().filter(f).join().build();
...
new Suite('filterMap')
  ...
  .on('complete', function () {
    // @ts-expect-error -- `this` is the Benchmark.Suite instance
...
new Suite('cached')
  .add('turbo cached: filterJoin', function () {
    const lFilterJoin = turbo<number>('xxx').filter(f).join().build();
    lFilterJoin(data);
  })
  .add('vanilla: filterJoin', function () {
    data.filter(f).join();
